            ...
            <div className="rounded-3xl border border-primary/20 bg-gradient-to-br from-primary/10 via-pastel-lilac to-pastel-blue p-6 md:p-8 shadow-[0_20px_60px_-25px_rgba(30,58,138,0.35)]">
              ...
              {expanded && (
                <div className="mt-6 animate-in fade-in slide-in-from-bottom-2 duration-500">
                  <div className="rounded-2xl border-2 border-primary/25 bg-gradient-to-br from-white via-pastel-lilac/60 to-pastel-peach/60 p-5 md:p-7 shadow-[0_10px_40px_-20px_rgba(30,58,138,0.35)]">
                    <div className="text-[10px] font-mono uppercase tracking-widest text-primary mb-3">
                      The bigger picture · your launchpad
                    </div>
                    <p className="text-sm md:text-base italic text-foreground/90 leading-relaxed">
                      {r.perspective.message}
                    </p>

                    <div className="mt-5 flex flex-wrap gap-x-8 gap-y-5 items-start">
                      {r.perspective.lessPrivileged && (
                        <div className="min-w-[140px]">
                          <div className="font-mono text-4xl md:text-5xl font-bold text-primary leading-none">
                            <AnimatedCounter value={r.perspective.lessPrivileged.number} duration={1800} />
                          </div>
                          <div className="text-[10px] font-mono uppercase tracking-widest text-foreground/70 mt-2">
                      ...
                      {r.perspective.facts.map((f, i) => (
                        <div key={i} className="min-w-[140px]">
                          <div className="font-mono text-3xl md:text-4xl font-bold text-primary leading-none">
                            <AnimatedCounter value={f.number} duration={1600} />
                          </div>
                          <div className="text-[10px] font-mono uppercase tracking-widest text-foreground/70 mt-2">{f.label}</div>
                        </div>
                      ))}
                    </div>
                    ...
                    <div className="mt-5 space-y-2 text-sm text-foreground/85 leading-relaxed">
                      {r.perspective.lessPrivileged && (
                        <p>
                          <span className="font-semibold text-foreground">{r.perspective.lessPrivileged.number}</span> {r.perspective.lessPrivileged.label} — {r.perspective.lessPrivileged.message}
                        </p>
                      )}
                      {r.perspective.facts.map((f, i) => (
                        <p key={i}>
                          <span className="font-semibold text-foreground">{f.number}</span> {f.label} — {f.detail}
                        </p>
                      ))}
                    </div>
                  </div>
