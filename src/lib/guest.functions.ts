...
- roleModels: MUST be INDIA-BASED, GENUINE, and PUBLICLY VERIFIABLE (real people with Wikipedia pages / mainstream press coverage — no fictional or unverifiable names). CRITICAL: match the user's EXACT age — pick people whose STANDOUT achievement happened at the user's current age (±2 years max). The user should look at each name and think "they did that when they were MY age". Never pick a role model whose breakthrough came 5+ years older than the user. In each `why`, explicitly mention the age at which that person's breakthrough happened so the age match is visible to the user.
