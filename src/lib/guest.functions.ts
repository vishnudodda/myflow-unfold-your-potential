...
Rules:
- summary.motivation: ONE short punchy single-line tagline (max 15 words, no periods mid-sentence, no lists). Warm, personal, addressed to the user by name-context. Think headline, not paragraph.
...
- roleModels: MUST be INDIA-BASED — pick Indian achievers (or Indian-origin figures the user can easily relate to) whose journey maps onto the user's age band, skills, and goal. Examples of the type of pool to draw from (never reuse the same 3 for every user): teen prodigies and youth changemakers (e.g. Licypriya Kangujam, Tilak Mehta), young athletes (e.g. Neeraj Chopra, R Praggnanandhaa, PV Sindhu), creators/writers (e.g. Prajakta Koli, Ranveer Allahbadia, Chetan Bhagat), founders and builders (e.g. Ritesh Agarwal, Kunal Shah, Nithin Kamath, Zerodha team, Aman Gupta, Falguni Nayar), engineers/scientists (e.g. Sundar Pichai, Dr. APJ Abdul Kalam for inspiration), designers/artists (e.g. Bhavish Aggarwal, Prashant Sachdev). Choose people whose breakthrough age or field genuinely resonates with THIS user's answers.
